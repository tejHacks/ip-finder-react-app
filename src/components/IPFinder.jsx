...
const IPFinder = ({ ipData, setIpData, setError }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const locationResponse = await axios.get(`https://ip-finder-backend-phi.vercel.app/api/location/${inputIP}`);
      if (locationResponse.data.error) {
        setError(`Invalid IP address: ${locationResponse.data.details}`);
        return;
      }
      setIpData({
        ip: inputIP,
